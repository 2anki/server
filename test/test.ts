...
import { DeckParser } from '../server/parser/DeckParser'
import Settings from '../server/parser/Settings'
...
test('Grouped cloze deletions', async (t) => {
  const deck = await getDeck('Grouped Cloze Deletions fbf856ad7911423dbef0bfd3e3c5ce5c 3.html', new Settings({ cherry: 'false', cloze: 'true' }))
  t.true(deck.name === 'Grouped Cloze Deletions')
  t.true(deck.cardCount === 10)
})

test('Colours', async (t) => {
  const deck = await getDeck('Colours 0519bf7e86d84ee4ba710c1b7ff7438e.html', new Settings({ cherry: 'false' }))
...
test('Nested Toggles', async (t) => {
  const deck = await getDeck('Nested Toggles.html', new Settings({ cherry: 'true' }))
  t.true(deck.cardCount === 6)
})

test('Global Tags', async (t) => {
  const deck = await getDeck('Global Tag Support.html', new Settings({ tags: 'true', cherry: 'false' }))
...
test('Cloze Deletions', async (t) => {
  const deck = await getDeck('Some Cloze Deletions 1a118169ada841a99a9aaccc7eaa6775.html', new Settings({ cherry: 'false' }))
